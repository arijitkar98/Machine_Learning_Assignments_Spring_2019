...
def get_best_split(dataset, classes, attributes_used):
    ...
    for i in range(total_words):
        if i+1 not in attributes_used:
            split_groups = node_split(i+1, dataset)
            info_gain = information_gain(dataset, split_groups, classes)
...
def build_DT(node, dataset, attributes_used, max_depth):
    ...
    if node.depth >= max_depth:
        ...
        build_DT(node.left, best_split_groups[0], attributes_used, max_depth)
        print(" ")
        if node.depth == 1:
            print(" ", end=" ")
        ...
        build_DT(node.right, best_split_groups[1], attributes_used, max_depth)
...
for max_depth in range(1,21):
    ...
    build_DT(root, train_docID_list, attributes_used, max_depth)
    print(" ")
    print("Decision Tree:\n")
...
maxdepth = [i for i in range(1,21)]
f = plt.figure()
